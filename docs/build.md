...
The result is a directory rootfs under `build/<board>/rootfs/rootfs`. The builder removes machine-id, SSH host keys and the dpkg/apt logs that record when each package was installed, enables the common services, creates a locked `root` account, installs the selected board's licensed `jh7110-pvr-rogue` package when it has been built, and installs `jh7110-prepare.service` plus the graphical setup (`jh7110-oobe`, its socket-activated root backend, and the greeter wrapper). `jh7110-console-setup.service` is installed but deliberately not enabled: it is the recovery path for a board whose desktop will not start. The logs go because a log line is content and the image passes that pin timestamps can only reach metadata; machine-id and host keys are per-installation secrets that systemd and the preparation unit create on the board. The builder also writes the board's own `/etc/hostname` and `/etc/hosts` with the same functions the board runs, over the copies mmdebstrap takes from the build host, so the image carries the board's name rather than the runner's.

On local first boot the preparation unit runs without a terminal - board hostname, timezone, locale, machine-id, SSH host keys, root filesystem growth, hardware report via `jh7110-info` - and the greeter session then runs the GTK setup wizard before the real greeter, because the image ships locked and with no loginable account, so the account has to exist before anything can log in. The wizard runs as `lightdm`, never as root; the privileged half is a fixed method table behind `/run/jh7110/oobe.sock`. The password is counted in characters rather than in bytes, so a short CJK password cannot pass an eight-character check, and it reaches `chpasswd` over a pipe rather than through a command line. The chroot also asserts the payload it just built - the wizard, the backend, the socket unit, the polkit rule, the greeter desktop file - so a rootfs that would come up without a way to create an account fails the build instead. See [first-boot.md](first-boot.md) and [oobe.md](oobe.md).
