...
The result is a directory rootfs under `build/<board>/rootfs/rootfs`. The builder removes machine-id, SSH host keys and the dpkg/apt logs that record when each package was installed, enables the common services, creates a locked `root` account, installs the selected board's licensed `jh7110-pvr-rogue` package when it has been built, and installs `jh7110-prepare.service` and `jh7110-console-setup.service`. The logs go because a log line is content and the image passes that pin timestamps can only reach metadata; machine-id and host keys are per-installation secrets that systemd and the preparation unit create on the board. The builder also writes the board's own `/etc/hostname` and `/etc/hosts` with the same functions the board runs, over the copies mmdebstrap takes from the build host, so the image carries the board's name rather than the runner's. On local first boot, the preparation unit runs without a terminal - board hostname, timezone, locale, machine-id, SSH host keys, root filesystem growth, hardware report via `jh7110-info` - and the console setup then shows an English `whiptail` screen that asks for and confirms a root password on the HDMI text console before LightDM starts; the desktop remains localized separately. The prompt runs before anything else that can fail, because the image ships root locked and a later failure would leave no usable account; the password length is counted in characters rather than in bytes, so a short CJK password cannot pass an eight-byte check under the `C` console locale.
